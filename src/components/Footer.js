      ...
      <div className="footer-inner">
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "10px" }}>
            {services.map((s) => (
              <li key={s.label}>
                <Link
                  href={s.href}
                  style={{ fontSize: "0.82rem", color: "#6A6560", textDecoration: "none", transition: "color 0.2s", lineHeight: 1.5 }}
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "10px" }}>
            {links.map((l) => (
              <li key={l.label}>
                <Link
                  href={l.href}
                  style={{ fontSize: "0.82rem", color: "#6A6560", textDecoration: "none", transition: "color 0.2s" }}
                  onMouseEnter={e => { e.target.style.color = "#C8A96E"; }}
                  onMouseLeave={e => { e.target.style.color = "#6A6560"; }}
                >
                  {l.label}
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            {[
              { icon: "M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0zM15 11a3 3 0 11-6 0 3 3 0 016 0z", text: "Caen", href: null },
              { icon: "M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z", text: "06 20 78 31 31", href: "tel:+33620783131" },
              { icon: "M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z", text: "gb.btp14@gmail.com", href: "mailto:gb.btp14@gmail.com" },
            ].map((item, i) => (
              <a
                key={i}
                href={item.href || undefined}
                style={{ display: "flex", gap: "10px", alignItems: "flex-start", textDecoration: "none", cursor: item.href ? "pointer" : "default" }}
              >
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#C8A96E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: "2px" }}>
                  <path d={item.icon} />
                </svg>
                <span style={{ fontSize: "0.8rem", color: "#6A6560", lineHeight: 1.6, whiteSpace: "pre-line" }}>{item.text}</span>
              </a>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
          {["Mentions légales", "Politique de confidentialité"].map((t) => (
            <Link
              key={t}
              href="#"
              style={{ fontSize: "0.72rem", color: "#3A3530", textDecoration: "none", letterSpacing: "0.05em", transition: "color 0.2s" }}
